...
describe('NewUserRoleDropdown', () => {
  ...
  test('it should display correct roles in dropdown', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('_feature_flag_manage_role')).toBeInTheDocument()
      expect(screen.getByText('_pipeline_executor')).toBeInTheDocument()
      expect(screen.getByText('_project_admin')).toBeInTheDocument()
    })

    expect(screen.getByText('_feature_flag_manage_role').closest('a')).not.toHaveClass('bp3-disabled')
    expect(screen.getByText('_pipeline_executor').closest('a')).not.toHaveClass('bp3-disabled')
    expect(screen.getByText('_project_admin').closest('a')).not.toHaveClass('bp3-disabled')
  })
  ...
  test('it should show plan enforcement popover for "Feature Flags Manage Role" when plan enforcement enabled', async () => {
    ...
    fireEvent.mouseOver(screen.getByText('_feature_flag_manage_role'))

    await waitFor(() => expect(screen.getByText('common.feature.upgradeRequired.pleaseUpgrade')).toBeInTheDocument())

    expect(screen.getByText('_feature_flag_manage_role').closest('a')).toHaveClass('bp3-disabled')
    expect(screen.getByText('_pipeline_executor').closest('a')).not.toHaveClass('bp3-disabled')
    expect(screen.getByText('_project_admin').closest('a')).not.toHaveClass('bp3-disabled')
  })
  ...
  test('it should show plan enforcement popover for FF management when plan enforcement enabled and limits reached', async () => {
    ...
    userEvent.click(screen.getByText('_feature_flag_manage_role'))
