...
const HomePage: React.FC = () => {
  ...
  const {
    ...
    // Inferred from RestfulProvider in index.js
    path: 'gatweay/dashboard/v1/search',
    queryParams: {
      accountId: accountId,
  ...
  const { data: tagsList, loading: fetchingTags } = useGet({
    // Inferred from RestfulProvider in index.js
    path: 'gatweay/dashboard/v1/tags',
    queryParams: {
      accountId: accountId,
      folderId: folderId === 'shared' ? '' : folderId
    }
  ...
  const { data: folderDetail } = useGet({
    // Inferred from RestfulProvider in index.js
    path: 'gatweay/dashboard/folderDetail',
