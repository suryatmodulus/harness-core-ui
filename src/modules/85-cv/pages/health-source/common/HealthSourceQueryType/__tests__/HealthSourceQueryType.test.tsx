...
describe('RuntimeInput Tests for RadioGroup', () => {
  ...
  test('should select default value as host based', async () => {
    ...
    const { container, getByTestId } = render(<SampleComponent onSubmit={onSubmit} initialValue={QueryType.HOST_BASED}/>)
    await act(async () => {
      fireEvent.click(getByTestId('submitButtonJest'))
    })
  ...
  test('should change to service host based', async () => {
    ...
    const { getByTestId, getByText } = render(<SampleComponent onSubmit={onSubmit} initialValue={QueryType.SERVICE_BASED}/>)
    await act(async () => {
