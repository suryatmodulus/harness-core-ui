...
export function MultiSelectEntityReference<T extends Identifier>(
  ...
  const getScopeAndUuidFromSelectDTO = (onSelectDTO: MultiSelectPayLoad<T>): ScopeAndIdentifier[] => {
    ...
    if (selectedItemsUuidAndScope && data) {
      ...
      selectedItemsUuidAndScope.forEach(el => {
        if (el.scope === selectedScope) {
          const item: any = data.find(_el => _el.identifier === el.identifier)?.record
          if (
            item &&
            tempCheckedItems[el.scope].items.findIndex((_el: any) => _el.identifier === item.identifier) === -1 &&
